...
|更改选区|Alt-S-T
|剪切蒙版|Ctrl-Alt-G
|快速填充|alt + delete 快速填充前景色，shift + backspace 选择填充前景色还背景色
|反相|Ctrl + i，色相反转，彩色照片和底片色就是反相，黑变白，蓝变黄、红变绿
|反选|Shift + Ctrl + I
|合并拷贝|Shift + Ctrl + C
|盖印所选图层|Ctr + Alt + E，合并选中的图层但不会覆盖原图层
|盖印所有图层|Ctrl + Alt + shift + E，合并所有可见图层但不覆盖原图层
|合并图层|Shift + Ctrl + E，就是合并图层并覆盖原图；
