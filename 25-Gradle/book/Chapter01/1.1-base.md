...
在Gradle中，最重要的两个概念是项目（Project）和任务（Task），每个build.gradle就是一个项目。每次构建至少包括一个项目，每一个项目包括一个或者多个任务，因此，Android中每一个待构建的工程是一个Project，构建一个Project需要执行一系列Task，比如编译、打包这些构建过程的子过程都对应着一个Task。具体来说，一个apk文件的构建包含以下Task：Java源码编译、资源文件编译、Lint检查、打包以生成最终的apk文件等等。
