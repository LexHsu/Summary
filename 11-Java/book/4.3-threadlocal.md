...
1. 在同一个线程内，完全不相关的两个段代码、方法之间共享一个变量
1. 在同一个线程内，虽然成员变量是共享的，但ThreadLocal可避免通过方法传参的方式传递成员变量。

ThreadLocal 不是用来解决共享对象的多线程访问问题的，其与synchronized完全没有可比性：

1. ThreadLocal解决同一线程内的资源共享问题。
2. synchronized 解决的是多个线程间的资源共享问题

ThreadLocal不是用来解决对象共享访问问题的，而主要是提供了保持对象的方法和避免参数传递：
...
通过threadLocal变量作为key来寻找本线程中所使用的一个实例，减少同一个线程中的参数的传递。
一般情况下，通过ThreadLocal.set() 到线程中的对象是该线程自己使用的对象，其他线程不需要访问，也访问不到。各个线程中访问的是不同的对象。
...
可见，向ThreadLocal里面存东西就是向它里面的ThreadLocalMap存东西，然后ThreadLocal把这个Map挂到当前的线程底下，这样Map就只属于该线程。
