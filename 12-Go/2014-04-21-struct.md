...
u3 := User{name: "Tom", age := 20}   // OK
u5 := User{name: "Tom", age := 20,}  // OK
u4 := User{
	name: "Tom",
...
- 面向对象三大特征，Go仅支持封装，不支持继承，多态。也没有class关键字，但可通通过struct的匿名字段方式，以及Go中的“方法”实现，具体见下文method一节。
...
// Go struct与C struct内存布局相同，无任何附加的object信息
// 上述代码中，变量m, u的内存布局：
