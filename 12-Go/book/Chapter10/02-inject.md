...
在 inject 中，被调用者为 func，因此注入属性就是对 func 注入实参，inject 也可以注入 struct，这样的话注入的属性就是 struct中 的已添加 tag 为 inject 的导出字段。
...
上面的例子中，定义了方法 Say 并在 main 方法中手动调用。但有的场景下，在 web 开发中，注册路由，服务器接受请求，然后根据 request path 调用相应的 handler。这个 handler 不是由手动来调用的，而是由服务器端根据路由匹配来查找对应的 handler 并自动调用。inject便由此而来，尝试用 inject 改写上面的代码：
