...
用“verb + modifier + object”语法更高效，即动词+修饰语+对象。

```
- some verbs: v (visual), c (change), d (delete), y (yank/copy).
- some modifiers: i (inside), a (around, append), t (till..finds a character),
                  f (find..like till except including the char), / (search..find a string/regex)
- some text objects: w (word), s (sentence) p (paragraph) b (block/parentheses), t (tag, works for html/xml)

don’t try to do anything by pressing keys many times, instead speak to the editor in sentences
...
###### 高效编辑
- `di"` 光标在""之间，则删除""之间的内容，否则删除最近的""之间的内容
